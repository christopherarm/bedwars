...
public class TNTSheepRegister implements ITNTSheepRegister {
    ...
    @Override
    public ITNTSheep spawnCreature(Bedwars bedwars,
            Sophiaee specialItem,
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                sheep.getTNT().setYield((float) (sheep.getTNT().getYield() * 1.0D));
                sheep.getTNT().setFuseTicks((int) Math.round(8*20));
                sheep.getTNT().setIsIncendiary(false);

                specialItem.updateTNT();
            }
        }.runTaskLater(bedwars, 5L);
